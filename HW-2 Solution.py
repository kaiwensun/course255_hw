...
# coding: utf-8
# Name: Kaiwen Sun
# Email: kas003@ucsd.edu
# PID: A53091621

# THIS IS THE SUBMISSION FOR FINAL TEST
# # Homework 2
# 
...
#filename = "../Data/hw2-files-final.txt"
filename = "../Data/hw2-files-1gb.txt"
...
tweets_rdd = sc.textFile(','.join(files)).map(lambda line:line.encode('utf-8')).cache()
print_count(tweets_rdd)
...
def safe_parse(raw_json):
    try:
        cooked_json = ujson.loads(raw_json)
        if not cooked_json.has_key('text') or not cooked_json.has_key('user'):
            return None
        return cooked_json
    except ValueError:
        return None
...
# your code here
tweets_json_rdd = tweets_rdd.map(safe_parse).filter(lambda j:j is not None).cache()
...
usrid_rdd = tweets_json_rdd.map(lambda j:j['user']['id_str'])
distinct_usrid_rdd = usrid_rdd.distinct()
print_users_count(distinct_usrid_rdd.count())
...
pickle_filename = "../Data/users-partition.pickle"
#pickle_dict = sc.pickleFile(pickle_filename)
import pickle
...
def lookupGroup(partition_dict,usrid_str):
    default_group_id = 7
    try:
        return partition_dict[usrid_str]
    except KeyError:
        return default_group_id

group_1_pair_rdd = usrid_rdd.map(lambda usrid_str:(lookupGroup(partition_dict,usrid_str),1))
group_count_pair_rdd = group_1_pair_rdd.reduceByKey(lambda x,y:x+y)
...
# your code here
group_count_pair_lst = group_count_pair_rdd.sortByKey().collect()
print_post_count(group_count_pair_lst)
...
def print_count_set(s):
    print 'Number of elements:', len(s)

usrid_wordset_pair_rdd = tweets_json_rdd.map(lambda j:(j['user']['id_str'],set(tok.tokenize(j['text'].encode('utf-8'))))).reduceByKey(lambda s1,s2:s1.union(s2)).cache()

token_lst = usrid_wordset_pair_rdd.flatMap(lambda pair:pair[1]).distinct()
print_count(token_lst)

#set version
#token_set = usrid_wordset_pair_rdd.map(lambda pair:pair[1]).reduce(lambda set1,set2:set1.union(set2))
#print_count_set(token_set)


#oldest version changed
# your code here
#usrid_textset_pair_rdd = tweets_json_rdd.map(lambda j:(j['user']['id_str'],set(j['text'].encode('utf-8')))).cache()
#text_rdd = usrid_textset_pair_rdd.map(lambda pair:pair[0]) #a list of all text
#token_rdd = text_rdd.flatMap(tok.tokenize)
##distinct_token_rdd = token_rdd.distinct() #a list of distinct tokeons
#print_count(distinct_token_rdd) # print number of distinct tokens
...
# your code here
#usrid_wordset_pair_rdd = tweets_json_rdd.map(lambda j:(j['user']['id_str'],set(tok.tokenize(j['text'].encode('utf-8'))))).reduceByKey(lambda s1,s2:s1.union(s2)).cache()
word_mntbyusr_pair_rdd = usrid_wordset_pair_rdd.flatMap(lambda pair:[(word,1) for word in pair[1]]).reduceByKey(lambda cnt1,cnt2:cnt1+cnt2)
word_mntbyusratleast100_pair_rdd = word_mntbyusr_pair_rdd.filter(lambda pair:pair[1]>=100).cache() # a list of (word,times_mentioned_by_user) pair where the times is at least 100
srted20_word_mntbyusr_pair_lst = word_mntbyusratleast100_pair_rdd.takeOrdered(20,lambda pair:(-pair[1],pair[0]))

print_count(word_mntbyusratleast100_pair_rdd)
print_tokens(srted20_word_mntbyusr_pair_lst)
...
#(distinctusrid,wordset)
#word100_set = set(word_mntbyusratleast100_pair_rdd.map(lambda pair:pair[0]).collect())

word_mntbyusratleast100_pair_dic = dict(word_mntbyusratleast100_pair_rdd.collect())

groupid_wordset_pair_rdd = usrid_wordset_pair_rdd.map(lambda (usrid,wordset):(lookupGroup(partition_dict,usrid),wordset))
groupidword_1_pair_rdd = usrid_wordset_pair_rdd.flatMap(lambda (usrid,wordset):[((lookupGroup(partition_dict,usrid),word),1) for word in wordset if word in word_mntbyusratleast100_pair_dic])
groupidword_cnt_pair_rdd = groupidword_1_pair_rdd.reduceByKey(lambda cnt1,cnt2:cnt1+cnt2)
for groupid in range(8):
    word_rel_pair_rdd = groupidword_cnt_pair_rdd.filter(lambda ((gid,word),cnt):gid==groupid).map(lambda ((gid,word),cnt):(word,get_rel_popularity(cnt,word_mntbyusratleast100_pair_dic[word])))
    srt10_word_rel_pair_lst = word_rel_pair_rdd.takeOrdered(10,lambda pair:(-pair[1],pair[0]))
    print_tokens(srt10_word_rel_pair_lst,groupid)
